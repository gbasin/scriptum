...
async fn create_sync_session_returns_expected_contract() {
    ...
    assert_eq!(session_store.workspace_for_session(parsed.session_id).await, Some(workspace_id));
    assert_eq!(
...
async fn run_websocket_load_stress_profile(profile: LoadStressProfile) {
    ...
    let total_updates = (profile.updates_per_second as u64).saturating_mul(profile.soak_seconds);
    let pacing_interval =
    ...
    let error_rate =
        if total_updates == 0 { 0.0 } else { (ack_error_count as f64) / (total_updates as f64) };
...
async fn run_websocket_reconnect_storm_profile(profile: ReconnectStormProfile) {
    let Some(database_url) = env::var("SCRIPTUM_RELAY_TEST_DATABASE_URL").ok() else {
        eprintln!("skipping websocket reconnect-storm test: set SCRIPTUM_RELAY_TEST_DATABASE_URL");
        return;
    };
    ...
    ws_send(&mut writer_socket, &WsMessage::Subscribe { doc_id, last_server_seq: Some(0) }).await;
...
async fn subscribe_tracks_subscription_and_sends_snapshot_and_updates() {
    ...
    match &messages[1] {
        WsMessage::YjsUpdate { doc_id: update_doc_id, base_server_seq, payload_b64, .. } => {
            assert_eq!(*update_doc_id, doc_id);
...
async fn awareness_aggregate_excluding_omits_sender() {
    ...
    let excluding_a = awareness_store.aggregate_excluding(workspace_id, doc_id, session_a).await;
