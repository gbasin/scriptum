...
fn websocket_resume_tokens_are_single_use_and_bound_to_session_context() {
    assert!(
        WS_SOURCE.contains("hello_ack_rotates_resume_token_and_enforces_single_use"),
        "resume tokens should rotate and enforce single-use semantics"
    );
    assert!(
        WS_SOURCE.contains("resume_token_is_bound_to_session_context"),
        "resume tokens must be bound to session context"
    );
    assert!(
        WS_SOURCE.contains("resume_accepted"),
...
fn expired_jwts_and_session_tokens_are_rejected() {
    ...
    assert!(
        WS_SOURCE.contains("hello_rejects_expired_session_token"),
